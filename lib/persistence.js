 ...
 * ---
 *
 * FUNCTIONAL PATTERNS (adapter implementations can choose to support):
 *
 *   Adapters may implement functional patterns using the Either monad
 *   (provided by the framework) and pure function composition. This is
 *   an implementation detail of the adapter, NOT part of the framework
 *   contract.
 *
 *   Example functional pattern an adapter could implement:
 *     createFunctionalRepository(schema, entity) -> repository with Either
 *     createQueryBuilder() -> composable query builder
 *
 *   These are ADAPTER-SPECIFIC. The framework doesn't define them.
 *
 * ---
 *
